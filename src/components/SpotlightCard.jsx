...
import { ThemeContext } from '../App'; // Tema context'ini içe aktarıyoruz
import './SpotlightCard.css';

// Hex rengini "R, G, B" formatına çeviren yardımcı fonksiyon
const hexToRGB = (hex) => {
...
export default function SpotlightCard({
  ...
  const glowColorRGB = hexToRGB(theme.hex || '#10b981'); // Varsayılan renk veya tema rengi
  ...
  const initializeParticles = useCallback(() => {
    ...
    memoizedParticles.current = Array.from({ length: particleCount }, () =>
      createParticleElement(Math.random() * width, Math.random() * height, glowColorRGB)
    );
    particlesInitialized.current = true;
  }, [particleCount, glowColorRGB]);
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      ripple.style.cssText = `
        ...
        background: radial-gradient(circle, rgba(${glowColorRGB}, 0.4) 0%, rgba(${glowColorRGB}, 0.2) 30%, transparent 70%);
  ...
  }, [animateParticles, clearAllParticles, enableTilt, enableMagnetism, clickEffect, glowColorRGB]);

  return (
    <div
      ref={cardRef}
      className={`spotlight-card ${className}`}
      style={{ '--glow-color-rgb': glowColorRGB }}
    >
      {children}
    </div>
  );
